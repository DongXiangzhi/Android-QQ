...
import cn.edu.ldu.qqclient.util.User;
        ...
        while (true) { //循环处理收到的各种消息
            ...
            if (msg.getType().equalsIgnoreCase("M_LOGIN")) { //是M_LOGIN消息 
                ...
                if (!MemberManager.userLogin(bean)) {//登录不成功
                    ...
                    Member backBean=MemberManager.getRowById(Integer.parseInt(userId));
                    backMsg.setUserId(userId);
                    backMsg.setNickName(backBean.getName());
                    backMsg.setPassword(backBean.getPassword());
                    backMsg.setHeadImage(backBean.getHeadImage());
                    byte[] buf=Translate.ObjectToByte(backMsg);
                    DatagramPacket backPacket=new DatagramPacket(buf,buf.length,packet.getAddress(),packet.getPort());//向登录用户发送的报文
                    serverSocket.send(backPacket); //发送   
                    ...
                    //更新服务器聊天室大厅
                    parentUI.txtArea.append(userId+" 远程地址："+packet.getSocketAddress()+" 登录！\n");
...
//                    }//end for                  
            ...
            }else if (msg.getType().equalsIgnoreCase("M_REGISTER")) { //是M_REGISTER消息 
                Message backMsg=new Message();
                Member bean=new Member();
                bean.setId(Integer.parseInt(userId));
                bean.setPassword(msg.getPassword());
                bean.setName(msg.getNickName());
                bean.setEmail(msg.getEmail());
                bean.setHeadImage(msg.getHeadImage());
                bean.setTime(new Timestamp(Calendar.getInstance().getTime().getTime()));
                
                if (!MemberManager.registerUser(bean)) {//注册不成功
                    backMsg.setType("M_REGISTER_LOST");
                    byte[] buf=Translate.ObjectToByte(backMsg);
                    DatagramPacket backPacket=new DatagramPacket(buf,buf.length,packet.getAddress(),packet.getPort());//向登录用户发送的报文
                    serverSocket.send(backPacket); //发送                  
                }else { //注册成功
                    backMsg.setType("M_REGISTER_DONE");
                    Member backBean=MemberManager.getRowById(Integer.parseInt(userId));
                    backMsg.setUserId(userId);
                    backMsg.setNickName(backBean.getName());
                    backMsg.setPassword(backBean.getPassword());
             ...
             }else if (msg.getType().equalsIgnoreCase("M_PMSG")) { //是M_PMSG消息
            //更新显示
            parentUI.txtArea.append(userId+"对"+msg.getTargetId()+" 说："+msg.getText()+"userList.size()"+userList.size()+"\n");
            
            //发给目标用户
            for (int i=0;i<userList.size();i++) { //遍历用户
                 parentUI.txtArea.append("枚举："+msg.getUserId()+"  "+userList.get(i).getPacket().getAddress()+"端口："+userList.get(i).getPacket().getPort()+"\n");
                if (userList.get(i).getUserId().equals(msg.getTargetId())) {  //找到目标用户
                DatagramPacket oldPacket=userList.get(i).getPacket();
                DatagramPacket newPacket=new DatagramPacket(data,data.length,oldPacket.getAddress(),oldPacket.getPort()); 
                serverSocket.send(newPacket); //发送
                parentUI.txtArea.append("发送给："+msg.getTargetId()+oldPacket.getAddress()+"端口："+oldPacket.getPort()+"\n");
                } 
            }
            }//end if M_PMSG 消息
